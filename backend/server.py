...
from database import Target as DBTarget
...
try:
    ...
    logger.info(
        "This might be due to database not being ready yet. The application will retry on first request."
    )
...
class VulnerabilityCreate(BaseModel):
    title: str = Field(
        ..., min_length=1, max_length=200, description="Vulnerability title"
    )
    payload: str = Field(
        ..., min_length=1, max_length=5000, description="Exploit payload"
    )
    how_it_works: str = Field(
        ..., min_length=1, max_length=10000, description="Vulnerability explanation"
    )
...
class VulnerabilityUpdate(BaseModel):
    title: Optional[str] = Field(
        None, min_length=1, max_length=200, description="Vulnerability title"
    )
    payload: Optional[str] = Field(
        None, min_length=1, max_length=5000, description="Exploit payload"
    )
    how_it_works: Optional[str] = Field(
        None, min_length=1, max_length=10000, description="Vulnerability explanation"
    )
...
async def run_nmap(target: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise Exception(
            "Nmap tool not found. Please ensure tools container is running."
        )
...
async def run_nuclei(target: str) -> Dict[str, Any]:
    ...
    try:
                            ...
                            if line.strip():
                                try:
                                    ...
                                    vulnerabilities.append(
                                        {
                                            "severity": vuln_data.get("info", {}).get(
                                                "severity", "medium"
                                            ),
                                            "template": vuln_data.get(
                                                "template_id", "nuclei"
                                            ),
                                            "description": vuln_data.get(
                                                "info", {}
                                            ).get("description", "Vulnerability found"),
                                            "cve": vuln_data.get("info", {})
                                            .get("classification", {})
                                            .get("cve_id", []),
                                            "url": vuln_data.get(
                                                "matched_at", clean_target
                                            ),
                                        }
                                    )
        ...
        raise Exception(
            "Nikto tool not found. Please ensure tools container is running."
        )
    except subprocess.TimeoutExpired:
...
async def create_project(
    ...
    db_project = DBProject(
        ...
        team_members=(
            json.dumps(project_data.team_members) if project_data.team_members else None
        ),
    ...
    project_obj = Project(
        ...
        team_members=(
            json.loads(db_project.team_members) if db_project.team_members else []
        ),
        targets=[],
...
async def remove_target_from_project(
    ...
    target = (
        db.query(DBTarget)
        .filter(DBTarget.id == target_id, DBTarget.project_id == project_id)
        .first()
    )
    if not target:
...
async def get_project_notes(
    ...
    try:
        ...
        notes = (
            db.query(DBNote)
            .filter(DBNote.project_id == project_id, DBNote.user_id == current_user.id)
            .all()
        )
        note_objects = [
            Note(
...
async def get_note(
    note_id: str, current_user=Depends(get_current_user), db: Session = Depends(get_db)
):
    try:
        # Get note from SQLite for the current user
        note = (
            db.query(DBNote)
            .filter(DBNote.id == note_id, DBNote.user_id == current_user.id)
            .first()
        )
        if not note:
...
@api_router.get(
    "/projects/{project_id}/vulnerabilities", response_model=List[Vulnerability]
)
async def get_project_vulnerabilities(
    ...
    db_vulns = (
        ...
        .filter(DBVulnerability.project_id == project_id)
        .order_by(DBVulnerability.created_at.desc())
        .all()
    )
...
async def get_vulnerability(
    vuln_id: str, current_user=Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    vuln = (
        db.query(DBVulnerability)
        .filter(
            DBVulnerability.id == vuln_id, DBVulnerability.user_id == current_user.id
        )
        .first()
    )
    ...
    for tool_name in tools_status:
        try:
            if tool_name == "subfinder":
                subprocess.run(
                    ["subfinder", "-version"], capture_output=True, timeout=5
                )
...
async def get_dashboard_stats(db: Session = Depends(get_db)):
    ...
    # Get recent projects
    recent_db_projects = (
        db.query(DBProject).order_by(DBProject.updated_at.desc()).limit(5).all()
    )
    recent_projects = []
