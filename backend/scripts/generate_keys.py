...
async def main():
    ...
    parser.add_argument(
        "--tier",
        required=True,
        choices=["professional", "teams", "enterprise", "elite"],
        help="License tier",
    )
    parser.add_argument("--duration", required=True, type=int, help="Duration in days")
    parser.add_argument(
        "--count", required=True, type=int, help="Number of keys to generate"
    )
    ...
    logger.info(
        f"Generating {args.count} keys for tier={args.tier}, duration={args.duration}d"
    )
    logger.info("Raw keys (store securely; not saved in DB):\n")
    for _ in range(args.count):
        raw_key = await create_license_key(
            db, args.tier, args.duration, store_raw=False
        )
        logger.info(raw_key)
