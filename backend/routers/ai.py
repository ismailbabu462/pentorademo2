...
class ChatRequest(BaseModel):
    message: str = Field(
        ..., min_length=1, max_length=10000, description="User message to AI"
    )
    project_id: str | None = Field(None, description="Optional project ID for context")
    tool_output_id: str | None = Field(
        None, description="Optional tool output ID for context"
    )
...
from config import AI_CONFIG, OLLAMA_MODEL, OLLAMA_URL
...
def validate_project_access(project_id: str, user_id: str, db: Session) -> bool:
    ...
    try:
        ...
        result = db.execute(
            project_query, {"project_id": project_id, "user_id": user_id}
        ).fetchone()
...
def validate_tool_output_access(tool_output_id: str, user_id: str, db: Session) -> bool:
    ...
    try:
        ...
        result = db.execute(
            tool_query, {"tool_output_id": tool_output_id, "user_id": user_id}
        ).fetchone()
        ...
        if request.project_id:
            ...
            try:
                ...
                if project_result:
                    ...
                    project_name = re.sub(
                        r'[<>"\']', "", str(project_result.name or "")
                    )
                    project_desc = re.sub(
                        r'[<>"\']', "", str(project_result.description or "")
                    )
                    project_status = re.sub(
                        r'[<>"\']', "", str(project_result.status or "")
                    )
...
"""
                    ...
                    targets_result = db.execute(
                        targets_query, {"project_id": request.project_id}
                    ).fetchall()
                    ...
                    if targets_result:
                        ...
                        for target in targets_result:
                            target_type = re.sub(
                                r'[<>"\']', "", str(target.target_type or "")
                            )
                            target_value = re.sub(
                                r'[<>"\']', "", str(target.value or "")
                            )
                            target_desc = re.sub(
                                r'[<>"\']', "", str(target.description or "")
                            )
...
"""
                    ...
                    notes_result = db.execute(
                        notes_query, {"project_id": request.project_id}
                    ).fetchall()
...
"""
                    ...
                    vulns_result = db.execute(
                        vulns_query, {"project_id": request.project_id}
                    ).fetchall()
                    ...
                    if vulns_result:
                        ...
                        for vuln in vulns_result:
                            ...
                            vuln_how = re.sub(
                                r'[<>"\']', "", str(vuln.how_it_works or "")
                            )
                            vuln_severity = re.sub(
                                r'[<>"\']', "", str(vuln.severity or "")
                            )
...
"""
                    ...
                    tools_result = db.execute(
                        tools_query, {"project_id": request.project_id}
                    ).fetchall()
...
"""
                ...
                logging.error(
                    f"Project context error for user {current_user.id}: {str(e)}"
                )
                raise HTTPException(
                    status_code=500, detail="Failed to load project context"
                )
        ...
        if request.tool_output_id:
            ...
            if not validate_tool_output_access(
                request.tool_output_id, current_user.id, db
            ):
                raise HTTPException(
                    status_code=403, detail="Tool output not found or access denied"
                )
...
"""
                ...
                # Log error internally, don't expose details
                logging.error(
                    f"Tool output context error for user {current_user.id}: {str(e)}"
                )
                raise HTTPException(
                    status_code=500, detail="Failed to load tool output context"
                )
        ...
        ollama_request = {"model": MODEL_NAME, "prompt": full_prompt, "stream": False}
        ...
        if response.status_code != 200:
            raise HTTPException(
                status_code=500, detail=f"Ollama service error: {response.status_code}"
            )
        ...
        raise HTTPException(
            status_code=500, detail="AI service timeout. Please try again."
        )
    except Exception as e:
        # Log error internally, don't expose details
        logging.error(f"AI Chat Error for user {current_user.id}: {str(e)}")
        raise HTTPException(status_code=500, detail="AI service error occurred")
