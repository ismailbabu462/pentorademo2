...
class ToolOutputRequest(BaseModel):
    tool_name: str = Field(
        ..., min_length=1, max_length=100, description="Name of the tool used"
    )
    target: str = Field(
        ..., min_length=1, max_length=500, description="Target of the scan"
    )
    output: str = Field(
        ..., min_length=1, max_length=100000, description="Tool output content"
    )
    status: str = Field(default="completed", description="Status of the tool execution")
    project_id: Optional[str] = Field(
        None, description="Optional project ID to link to"
    )
    ...
    def validate_target(cls, v):
        ...
        if len(target) > TOOL_CONFIG["max_target_length"]:
            raise ValueError(
                f'Target too long (max {TOOL_CONFIG["max_target_length"]} characters)'
            )
    ...
    def validate_output(cls, v):
        ...
        if len(v) > TOOL_CONFIG["max_output_size"]:
            raise ValueError(
                f'Output too large (max {TOOL_CONFIG["max_output_size"]} bytes)'
            )
...
async def save_tool_output(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to save tool output")
...
async def get_tool_outputs(
    ...
    try:
        if project_id:
            ...
            results = db.execute(
                query, {"project_id": project_id, "user_id": current_user.id}
            ).fetchall()
        else:
        ...
        raise HTTPException(status_code=500, detail="Failed to get tool outputs")
...
async def get_tool_output(
    ...
    try:
        ...
        if not re.match(r"^tool_\d{8}_\d{6}_[a-z0-9_]+$", output_id):
            raise HTTPException(status_code=400, detail="Invalid tool output ID format")
        ...
        result = db.execute(
            query, {"output_id": output_id, "user_id": current_user.id}
        ).fetchone()
        ...
        raise HTTPException(status_code=500, detail="Failed to get tool output")
