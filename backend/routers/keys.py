...
async def activate_key(
    ...
    try:
        ...
        if not incoming_key or len(incoming_key) < 20:
            logger.debug(
                f"License Key Activation: Invalid key format - length: {len(incoming_key)}"
            )
        ...
        if not matched_key:
            ...
            raise HTTPException(
                status_code=400, detail="Invalid or previously used key"
            )
...
async def get_latest_key_for_user(
    user_id: str = Depends(get_current_user_id), db: Session = Depends(get_db)
):
    # SECURITY: Find most recent unused key created for this specific user
    key = (
        db.query(DBLicenseKey)
        .filter(
            DBLicenseKey.is_used == False,
            DBLicenseKey.created_for_user_id
            == user_id,  # SECURITY: Ensure key belongs to requesting user
        )
        .order_by(DBLicenseKey.created_at.desc())
        .first()
    )

    if not key:
        raise HTTPException(
            status_code=404, detail="No pending license key found for user"
        )
    ...
    if key.created_for_user_id != user_id:
        raise HTTPException(
            status_code=403, detail="Access denied: Key does not belong to user"
        )
