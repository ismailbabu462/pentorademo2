...
def get_google_search_results(query: str, num_results: int = 5) -> List[Dict[str, str]]:
    ...
    try:
        ...
        logger.info(
            f"Found {len(search_results)} Google search results for query: {query}"
        )
...
Analysis:"""
        ...
        response = requests.post(
            f"{OLLAMA_URL}/api/generate", json=ollama_request, timeout=120
        )
    ...
    try:
        ...
        with DatabaseContext() as db:
            ...
            vulnerability = (
                ...
                .filter(DBVulnerability.id == vulnerability_id)
                .first()
            )
            ...
            self.update_state(
                state="PROGRESS",
                meta={
                    "status": "Searching for vulnerability information...",
                    "progress": 25,
                },
            )
            ...
            # Create search query
            search_query = (
                f'"{vulnerability.title}" exploit tutorial code vulnerability'
            )
