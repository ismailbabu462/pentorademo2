...
def handle_database_error(
    error: SQLAlchemyError, operation: str = "database operation"
) -> None:
    ...
    if "UNIQUE constraint failed" in str(error) or "Duplicate entry" in str(error):
        ...
        raise ValidationError(
            "Invalid reference to related resource", {"operation": operation}
        )
...
def create_error_response(
    ...
    if isinstance(error, APIError):
        response = {"error": error.message, "status_code": error.status_code}
        if include_details and error.details:
            response["details"] = error.details
    elif isinstance(error, HTTPException):
        response = {"error": error.detail, "status_code": error.status_code}
    else:
