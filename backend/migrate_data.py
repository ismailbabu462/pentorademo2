...
def main():
    ...
    try:
        ...
        for model_class, table_name in migration_plan:
            if migrate_table_data(
                sqlite_session, mysql_session, model_class, table_name
            ):
                success_count += 1
            else:
        ...
        logger.info(
            f"Migration completed: {success_count}/{total_tables} tables successful"
        )
        ...
        logger.info(
            f"Verification completed: {verification_success}/{total_tables} tables verified"
        )
